...
4) Call `AudioManager.Instance.Play("Sound Name")` to play looping/persistent sounds (can only play one instance of that sound at a time), or `AudioManager.Instance.SpawnSFX("Sound Name")` for sounds that can occur in rapid succession (creates a prefab for each instance and randomizes pitch/volume).
